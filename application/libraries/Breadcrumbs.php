...
class Breadcrumbs {
	...
	function show()
	{
		if ($this->breadcrumbs) {
			...
			$output .= $this->crumb_open.'<a href="' . base_url() . '">' . $this->ci->config->item('app_name') . '</a> '.$this->crumb_divider.$this->crumb_close;
			
			// construct output
			foreach ($this->breadcrumbs as $key => $crumb) {
				if (end(array_keys($this->breadcrumbs)) == $key) {
					$output .= $this->crumb_last_open . '<span>' . $crumb['page'] . '</span>' . $this->crumb_close;
				} else {
